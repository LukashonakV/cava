...
int audio_raw_init(struct audio_data *audio, struct audio_raw *audio_raw, struct config_params *prm,
    ...
    if (prm->fixedbars) {
        number_of_bars = prm->fixedbars;
        if (number_of_bars * prm->bar_width + prm->fixedbars * prm->bar_spacing - prm->bar_spacing >
            audio_raw->width) {
            cleanup(prm->output);
        ...
        if (audio_raw->output_channels == 2 && number_of_bars % 2 != 0) {
            cleanup(prm->output);
